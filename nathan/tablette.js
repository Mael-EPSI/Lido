...
function chargerProduits() {
    fetch("https://api-pizza.digitalprojects.fr/produits.php")
...
function chargerMenus() {
    fetch("https://api-pizza.digitalprojects.fr/get_menus.php")
        .then(response => response.json())
        .then(data => {
...
function chargerCommande() {
    fetch("https://api-pizza.digitalprojects.fr/get_commande.php")
        .then(response => response.json())
        .then(data => {
            if (data.success) {
    ...
    document.querySelectorAll('.section').forEach(section => {
        ...
        if (commandes[tableId] && commandes[tableId].length > 0) {
            commandes[tableId].forEach(commande => {
                ...
                // Création d'une section pour les menus
                let menusHTML = '';
                if (commande.menus && commande.menus.length > 0) {
                    menusHTML = `
                        <h4>Menus :</h4>
                        <ul>
                            ${commande.menus.map(menu => {
                                if (typeof menu === 'object' && menu.nom && menu.produits) {
                                    return `
                                        <li>
                                            ${menu.nom}
                                            <ul class="menu-products">
                                                ${menu.produits.map(produit => 
                                                    `<li class="menu-product">${produit}</li>`
                                                ).join('')}
                                            </ul>
                                        </li>
                                    `;
                ...
                // Création d'une section pour les produits hors menu
                let produitsHTML = '';
                ...
                total += commande.produits.length * 10;
                total += commande.menus.length * 15;
...
function envoyerCommande() {
    ...
    if (!tableId) {
        console.error("❌ Erreur : Table ID non définie.");
        alert("❌ Problème avec l'ID de la table.");
        return;
    }
    ...
    // 🔹 Récupérer uniquement les nouveaux produits ajoutés (ceux qui n'ont pas la classe 'commande-item')
    commandeContainer.querySelectorAll('.produit-item:not(.commande-item)').forEach(item => {
        const id = item.dataset.id;
        const prix = item.dataset.prix;
        if (!id || !prix) {
            console.error("❌ Produit sans ID ou prix détecté :", item);
            return;
        }
        produits.push({ id, prix });
    });

    // 🔹 Récupérer uniquement les nouveaux menus ajoutés
    commandeContainer.querySelectorAll('.menu-item:not(.commande-item)').forEach(item => {
        const id = item.dataset.id;
        const prix = item.dataset.prix;
        if (!id || !prix) {
            console.error("❌ Menu sans ID ou prix détecté :", item);
            return;
        }

        let produitsMenu = [];
        if (item.dataset.produits) {
            try {
                produitsMenu = JSON.parse(item.dataset.produits);
            } catch (error) {
                console.error("❌ Erreur lors du parse JSON de 'produits' pour le menu :", error);
            }
        }

        menus.push({ id, prix, produits: produitsMenu });
    });

    // 🔹 Vérifier s'il y a de nouveaux éléments à ajouter
    if (produits.length === 0 && menus.length === 0) {
        alert("❌ Aucun nouveau produit ou menu à ajouter !");
        return;
    }

    // 🔹 Récupérer l'ID de la commande existante si elle existe
    let commandeId = null;
    if (commandes[tableId] && commandes[tableId].length > 0) {
        commandeId = commandes[tableId][commandes[tableId].length - 1].commande_id;
    }
    ...
    const payload = {
        ...
        produits: produits,
        menus: menus,
        commande_id: commandeId // Ajout de l'ID de la commande existante
    };

    console.log("📤 Données envoyées à l'API :", JSON.stringify(payload, null, 2));

    // 🔹 Envoi des données à l'API
    fetch("https://api-pizza.digitalprojects.fr/envoyer_commande.php", {
        method: "POST",
        mode: "cors",
...
document.addEventListener('click', function (event) {
    if (event.target.classList.contains('payer-btn')) {
        ...
        if (button.dataset.confirmed === "true") {
            ...
            fetch("https://api-pizza.digitalprojects.fr/payer.php", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
...
function showToastNotification() {
    fetch('https://api-pizza.digitalprojects.fr/get_notification.php')
      .then(response => response.json())
      .then(data => {
        ...
        toast.addEventListener('click', () => {
          fetch('https://api-pizza.digitalprojects.fr/marquer_lu.php', {
            method: 'POST',
            headers: {
